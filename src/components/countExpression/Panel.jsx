...
import Cell from '@/components/countExpression/Cell.jsx';
...
class CountExpression extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.dimensionId !== this.props.dimensionId || nextProps.eventSourceId !== this.props.eventSourceId) {
            this.getComputeExpressionData(nextProps);
        }
    }
    getComputeExpressionData(props) {
        if (common.isEmpty(props.eventSourceId) || common.isEmpty(props.dimensionId)) return

        commonService.getInitData(props.dimensionId, props.eventSourceId, props.entityType, props.fieldList).then(res => {
            if (!publicUtils.isOk(res)) return
            let VAR_SELECTION = [], computeFunction = [];

            switch (this.props.entityType) {
                case 0://衍生变量
                    let fixVarSeletion_0 = (data) => {
                    ...
                    if (res.data.result.VAR_SELECTION_NUMBER) VAR_SELECTION = fixVarSeletion_0(res.data.result.VAR_SELECTION_NUMBER);
                    if (res.data.result.computeFunction) computeFunction = res.data.result.computeFunction;
                    break;
                case 1://实时查询变量
                    let fixVarSeletion_1 = (data) => {
                        data.forEach(element => {
                            element.value = element.type;
                            element.list.forEach(element2 => {
                                element2.value = element2.code;
                    ...
                    if (res.data.result.VAR_SELECTION) VAR_SELECTION = fixVarSeletion_1(res.data.result.VAR_SELECTION);
                    break;
                case 2://规则
                    let fixVarSeletion_2 = (data) => {
                        data.forEach(element => {
                            element.value = element.type;
                            element.list.forEach(element2 => {
                                element2.value = element2.code;
                    ...
                    if (res.data.result.VAR_SELECTION_NUMBER) VAR_SELECTION = fixVarSeletion_2(res.data.result.VAR_SELECTION_NUMBER);
                    break;
                default:
                    break;
            }

            this.setState({ loading: false, VAR_SELECTION, computeFunction })
        ...
        return (
            <div className="countExpression-cell-container">
                <Cell
                    ...
                    computeFunction={this.state.computeFunction}
...
CountExpression.propTypes = {
    entityType: PropTypes.oneOf([0, 1, 2, 3, 4, 5]),//实体类型 0衍生变量1实时查询变量2规则3策略4决策表5评分卡
