...
class Catalog extends React.Component {
    ...
    onDrop = (info) => {
        ...
        if (!info.dropToGap) {
            ...
            (info.node.props.child || []).length > 0 // Has child
        ...
        if (fromKey.substr(0, fromKey.length - 1) === toKey.substr(0, fromKey.length - 1) && fromKey.length === toKey.length) {
            ...
            if (keyLength === 1) {
                ...
                console.log("跨级拖动排序 fromKey = ", fromKey);
                let data;
    ...
    render() {
            ...
            if (item.child && item.child.length) {
                return <TreeNode
                    ...
                    title={
                        <div >
                            <span onClick={() => {
                                if (!publicUtils.isAuth('system:power:view')) return
        ...
        return (
            <Tree
                ...
                draggable={publicUtils.isAuth("system:power:edit") ? true : false}
