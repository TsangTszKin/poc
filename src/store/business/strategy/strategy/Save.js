 ...
 * @LastEditTime: 2019-08-12 16:51:10
 * @Description: 
 */
...
class store extends React.Component {
    ...
    type = '';//勿删，strategy特有的逻辑判断依据 路由type
    ...
    getTestInput(id) {
        ...
        commonService.getTestInput('strategy', id).then(this.getTestInputCallBack)
    }
    ...
    inputValueChange(index, value, dataType) {
        ...
        console.log("inputResponseData", this.inputResponseData)
    ...
    copyForApi() {
        common.loading.show();
        strategyService.strategyCopy(this.getId).then(this.copyForApiCallBak).catch(() => common.loading.hide());
    }
    ...
    setDefaultVersionForApi() {
        common.loading.show();
        strategyService.strategySetDefaultVersion(this.getId).then(this.setDefaultVersionForApiCallBack).catch(() => common.loading.hide());
    }
    ...
    allVersionForApi() {
        strategyService.strategyAllVersion(this.getId).then(this.allVersionForApiCallBack).catch(() => common.loading.hide());
    }
    ...
    approvalSubmitForApi() {
        common.loading.show();
        strategyService.strategySubmitInspect(toJS(this.approvalSubmitParams).id).then(this.approvalSubmitForApiCallBack).catch(() => common.loading.hide());
    }
    ...
    deleteVersionForApi(id) {
        common.loading.show();
        strategyService.deleteStrategy(id).then(this.deleteVersionForApiCallBack).catch(() => common.loading.hide());
    }
