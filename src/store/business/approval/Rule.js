 ...
 * @LastEditTime: 2019-03-13 11:40:30
 * @Description: 
 */
...
import TableAction from '@/components/business/approval/rule/TableAction';
...
class store {
    ...
    passForApi(i) {
        common.loading.show();
        approvalService.pass(this.resultDataList[i].ruleId, this.params.getType, this.resultDataList[i]).then(this.passForApiCallBack).catch(() => common.loading.hide())
    }
    ...
    rejectForApi(i) {
        common.loading.show();
        approvalService.reject(this.resultDataList[i].ruleId, this.params.getType, this.resultDataList[i]).then(this.rejectForApiCallBack).catch(() => common.loading.hide())
    }
