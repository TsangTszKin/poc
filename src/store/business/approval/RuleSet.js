 ...
 * @LastEditTime: 2019-03-13 11:39:37
 * @Description: 
 */
...
import TableAction from '@/components/business/approval/rule-set/TableAction';
...
const columns = [
    {
        ...
        key: 'ruleSetCode',
        sorter: (a, b) => {
            return a.ruleSetCode.localeCompare(b.ruleSetCode)
        }
    }, {
        title: '版本',
        dataIndex: 'version',
...
class store {
    ...
    passForApi(i) {
        common.loading.show();
        approvalService.pass(this.resultDataList[i].ruleSetId, this.params.getType, this.resultDataList[i]).then(this.passForApiCallBack).catch(() => common.loading.hide())
    }
    ...
    rejectForApi(i) {
        common.loading.show();
        approvalService.reject(this.resultDataList[i].ruleSetId, this.params.getType, this.resultDataList[i]).then(this.rejectForApiCallBack).catch(() => common.loading.hide())
    }
