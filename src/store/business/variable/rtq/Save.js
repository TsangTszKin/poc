 ...
 * @LastEditTime: 2019-08-12 16:53:54
 * @Description: 
 */
...
class store extends React.Component {
    ...
    getTestInput(id) {
        ...
        commonService.getTestInput('rtqVar', id).then(this.getTestInputCallBack)
    }
    ...
    copyForApi() {
        common.loading.show();
        variableService.rtqCopy(this.getId).then(this.copyForApiCallBak).catch(() => common.loading.hide());
    }
    ...
    setDefaultVersionForApi() {
        common.loading.show();
        variableService.rtqSetDefaultVersion(this.getId).then(this.setDefaultVersionForApiCallBack).catch(() => common.loading.hide());
    }
    ...
    allVersionForApi() {
        variableService.rtqAllVersion(this.getId).then(this.allVersionForApiCallBack);
    }
    @action.bound allVersionForApiCallBack(res) {
        ...
        res.data.result.forEach(element => {
            ...
            if (element.id === this.getId) {
                // alert(element.id + '   ' + this.getId + '    ' + element.version)
    ...
    deleteVersionForApi(id) {
        common.loading.show();
        variableService.deleteOneRtqVar(id).then(this.deleteVersionForApiCallBack).catch(() => common.loading.hide());
    }
