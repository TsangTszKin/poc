/*
 * @Author: zengzijian
 * @Date: 2019-01-14 09:23:33
 * @LastEditors: zengzijian
 * @LastEditTime: 2019-08-12 16:59:41
 * @Description: 
 */
...
class Store {
    ...
    getTreeForApi() {
        ...
        organizationService.getTree().then(this.getTreeForApiCallBack).catch(() => { this.loading.setLeft(false) });
    }
    @action.bound getTreeForApiCallBack(res) {
        this.loading.setLeft(false);
        ...
        if (common.isEmpty(res.data.result)) {
            ...
            this.tree.setDefaultSelectEdNodeKey(res.data.result[0].id);
        }
    }

    getDetailsByIdForApi(id) {
        common.loading.show();
        organizationService.getDetailsById(id).then(this.getDetailsByIdForApiCallBack).catch(() => common.loading.hide())
    }
    ...
    saveDetailsForApi() {
        ...
        common.loading.show();
        organizationService.saveDetails(data).then(this.saveDetailsForApiCallBack).catch(() => common.loading.hide());
    }
    ...
    deleteOneForApi(id) {
        ...
        Modal.confirm({
            ...
            onOk() {
                console.log('OK');
                common.loading.show();
                organizationService.deleteOne(id).then(self.deleteOneForApiCallBack).catch(() => common.loading.hide())
            },
            onCancel() {
                console.log('Cancel');
            },
    ...
    moveNodeForApi(id, parentId, ids) {
        common.loading.show();
        organizationService.moveNode(id, parentId, ids).then(this.moveNodeForApiCallBack).catch(() => common.loading.hide())
    }
    ...
    getEventSourceSelectListForApi() {
        eventSourceService.getAuthedEventSourceList({ query: '', page: 1, size: 1000 }).then(this.getEventSourceSelectListForApiCallBack)
    }
    @action.bound getEventSourceSelectListForApiCallBack(res) {
        if (!publicUtils.isOk(res)) return
        if (!common.isEmpty(res.data.pageList.resultList)) {
            this.setEventSourceSelectList(res.data.pageList.resultList);
        } else {
