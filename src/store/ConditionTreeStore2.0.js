 ...
 * @LastEditTime: 2019-08-12 16:44:57
...
class ConditionTreeStore {
    ...
    @action addNode(nodeKey, type, expressionVO, isBranch, extraType) {
        let self = this;
        let temp = {
            expressionVO: {
                "varCategoryType": 1,
        ...
        // 简单决策表就不让他修改变量，直接将原来的变量赋值到新的节点
        if (['easyStrategyTable'].includes(extraType)) {
            const { varName, varCode, varType, varDataType,  } = expressionVO;
            Object.assign(temp.expressionVO, {
                varName,
                varCode,
                varType,
                varDataType,
    ...
    @action updateNodeData = (nodeKey, name, value) => {
        ...
        if (name !== 'relType') {
            ...
            console.log("···············修改后的 conditionsAll ··············", conditionsAll)
            this.setConditionsAll(conditionsAll);
